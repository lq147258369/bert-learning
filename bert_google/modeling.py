...
class BertConfig(object):
    ...
    def __init__(self,
                 ...
                 intermediate_size=3072,  # e n c o d e r的“中间”隐层神经元数（例如f e e d -f o r wa r d l a y e r）
                 hidden_act="gelu",
    ...
    Args:
      ...
      initializer_range: The stdev of the truncated_normal_initializer for
        ...
        bert参数计算
            ...
            embeddings.position_embeddings.weight	[512, 768]	393,216                                   第一行是代表第一个序列的第一个位置，第二行代表序列的第二个位置，以此类推。因此，如果有这样两个句子“Hello world” 和“Hi there”, “Hello” 和“Hi”会由完全相同的position embeddings，因为他们都是句子的第一个词。同理，“world” 和“there”也会有相同的position embedding。
            embeddings.token_type_embeddings.weight	[2, 768]	1,536                                    区分一个句子对中的两个句子:[0000000000 1111111111111].只有两种向量表示。前一个向量是把0赋给第一个句子中的各个token, 后一个向量是把1赋给第二个句子中的各个token。如果输入仅仅只有一个句子，那么它的segment embedding就是全0。
...
def embedding_lookup(input_ids,
  ...
  Args:
    input_ids: int32 Tensor of shape [batch_size, seq_length] containing word ids.
      # 该函数默认输入的形状为【b a t c h _ s i z e , s e q _ l e n g t h , i n p u t _ n u m 】
            # 如果输入为2 D 的【b a t c h _ s i z e , s e q _ l e n g t h 】，
            # 则扩展到【b a t c h _ s i z e , s e q _ l e n g t h , 1 】
  ...
  """
    ...
    # o u t p u t ：[ b a t c h _ s i z e , s e q _ l e n g t h , n u m _ i n p u t s ]
    # 转成: [ b a t c h _ s i z e , s e q _ l e n g t h , n u m _ i n p u t s * e m b e d d i n g _ s i z e ]
...
def embedding_postprocessor(input_tensor,
    ...
    """Performs various post-processing on a word embedding tensor.
        在 Tr a n sf o rme r 论 文 中 的
        p o s i t i o n e m b e d d i n g 是 由 si n / c o s 函 数 生 成 的 固 定 的 值 ， 而 在 这 里 代 码 实 现 中 是 跟 普
        通wo r d emb e d d i n g一样随机生成的，可以训练的。作者这里这样选择的原因可能是BERT
        训练的数据比Tr a n sf o rme r那篇大很多，完全可以让模型自己去学习
  Args:
  ...
  """
    ...
    if use_position_embeddings:
        assert_op = tf.assert_less_equal(seq_length,
                                         max_position_embeddings)  # 如果seq_length>max_position_embeddings就抛出异常
