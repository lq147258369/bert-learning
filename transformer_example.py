...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, enc_inputs):                                  # enc_inputs: [batch_size, seq_len, d_model]
        enc_inputs += self.pos_table[:enc_inputs.size(1), :]        # enc_inputs加上PositionalEncoding
...
class Encoder(nn.Module):
    def __init__(self):
        super(Encoder, self).__init__()                                          # nn.Module的子类函数必须在构造函数中执行父类的构造函数 # 等价与nn.Module.__init__()
...
class DecoderLayer(nn.Module):
    ...
    def forward(self, dec_inputs, enc_outputs, dec_self_attn_mask,
                dec_enc_attn_mask):                                             # dec_inputs: [batch_size, tgt_len, d_model]
                                                                                # enc_outputs: [batch_size, src_len, d_model]
                                                                                # dec_self_attn_mask: [batch_size, tgt_len, tgt_len] 三角mask
        # 看论文的里maked multi-head attention                                    # dec_enc_attn_mask: [batch_size, tgt_len, src_len]
...
class Decoder(nn.Module):
    ...
    def forward(self, dec_inputs, enc_inputs, enc_outputs):                         # dec_inputs: [batch_size, tgt_len]
         ...
         # embedding+position embedding                                                                           # enc_outputs: [batsh_size, src_len, d_model]
        ...
        dec_self_attn_mask = torch.gt((dec_self_attn_pad_mask +                         #torch.gt(a,b)函数比较a中元素大于（这里是严格大于）b中对应元素，大于则为1，不大于则为0，这里a为Tensor，b可以为与a的size相同的Tensor或常数。
